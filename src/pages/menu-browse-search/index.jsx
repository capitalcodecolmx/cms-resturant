...
function MenuBrowseSearch() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        // Load restaurants
        const restaurantsResult = await menuService.getRestaurants();
        if (restaurantsResult.success) {
          setRestaurants(restaurantsResult.data || []);
          
          // Select first restaurant if available
          if (restaurantsResult.data?.length > 0) {
            const firstRestaurant = restaurantsResult.data[0];
            setSelectedRestaurant(firstRestaurant);
            
            // Load categories for first restaurant
            const categoriesResult = await menuService.getCategories(firstRestaurant.id);
  ...
  // Load menu items when restaurant or filters change
  useEffect(() => {
    ...
    const debounceTimer = setTimeout(loadMenuItems, 300);
    return () => clearTimeout(debounceTimer);
  }, [selectedRestaurant, selectedCategory, searchQuery, filters]);
  ...
  const getCategoryIcon = (categoryName) => {
    ...
    return iconMap[categoryName.toLowerCase()] || 'Circle';
  };

  // Filter and search logic
  const filteredItems = useMemo(() => {
    return menuItems; // Filtering is now done server-side
  }, [menuItems]);
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <div className="w-24 h-24 bg-error-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <Icon name="AlertCircle" size={32} className="text-error-500" />
          </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-body font-body-medium text-text-primary">
                  {selectedRestaurant?.name || currentLocation}
                </span>
              </div>
              <button
              ...
              <div className="bg-surface rounded-lg shadow-soft p-6 sticky top-32">
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {['vegetarian', 'vegan', 'gluten_free', 'protein_rich'].map(diet => (
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center space-x-3 mb-3">
                            ...
                            <div className="flex items-center space-x-1">
                              {[...Array(3)].map((_, index) => (
                                <Icon
                                  ...
                                  name="Flame"
                                  size={12}
                                  className={index < item.spice_level ? getSpiceLevelColor(item.spice_level) : 'text-secondary-200'}
                                />
                              ))}
                            </div>
                          )}
                          ...
                          <div className="flex items-center space-x-1 text-xs text-text-secondary">
                            ...
                            <span className="font-body">{item.prep_time}min</span>
                          </div>
                        </div>
